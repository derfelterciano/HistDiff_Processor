...
export function createHeatmapBuff(
	...
	const colorScale = d3.scaleSequential(d3.interpolateViridis).domain([contrast.min, contrast.max]);
...
export function blitBuffers(
	...
	ctx.drawImage(
		heatmapBuf,
		0, 0,
		heatmapBuf.width, heatmapBuf.height,
		treeWidth, 0,
		heatmapBuf.width * cellWidth, heatmapBuf.height * cellHeight
	);
}